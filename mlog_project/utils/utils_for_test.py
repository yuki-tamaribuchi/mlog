...
def create_test_artist(artist_name, slug, genre_name):
...
def create_test_song(song_name, artist_name, slug, genre_name):
	from musics.models import Song
	song_instance = Song.objects.create(
		song_name = song_name
	)
	song_instance.artist.add(create_test_artist(artist_name, slug ,genre_name))
...
def create_test_entry(title, content, username, handle, biograph, song_name, artist_name, slug, genre_name):
	from entry.models import Entry
	entry_instance = Entry.objects.create(
		title = title,
		content = content,
		writer = create_test_user(username, handle, biograph),
		song = create_test_song(song_name, artist_name, slug, genre_name)
	)
	return entry_instance

def create_test_comment(comment, username_for_comment, handle_for_comment, biograph_for_comment, title, content, username_for_entry, handle_for_entry, biograph_for_entry, song_name, artist_name, slug, genre_name):
